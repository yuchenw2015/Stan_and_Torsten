...
## Let's suppose that values < 0.5 are BQL

loq = 0.5 # Lower limit of quantitation
...
iBql <- with(xdata, (1:nrow(xdata))[DV < loq & EVID == 0])
nBql <- length(iBql)

xdata <- xdata %>%
  mutate(ifelse(DV < loq, NA, DV))
