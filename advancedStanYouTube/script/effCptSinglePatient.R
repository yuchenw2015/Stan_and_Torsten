...
p1 <- ggplot(xdata, aes(x = time, y = cobs))
...
p1 <- ggplot(xdata, aes(x = time, y = fxa.inh.obs))
p1 + geom_point() + geom_line()
    labs(x = "time (h)",
         y = "response") +
             theme(text = element_text(size = 12), axis.text = element_text(size = 12),
...
xplot <- xdata %>%
    mutate(xend = c(cobs[-1], NA), yend = c(fxa.inh.obs[-1], NA))
xplot <- xplot[-nrow(xplot),]
p1 <- ggplot(xplot, aes(x = cobs, y = fxa.inh.obs))
p1 + geom_segment(aes(xend = xend, yend = yend), arrow = arrow(angle = 15, ends = "last", type = "closed")) +
    labs(x = "plasma drug concentration (ng/mL)",
         y = "response") +
    theme(text = element_text(size = 12), axis.text = element_text(size = 12),
          legend.position = "none", strip.text = element_text(size = 8))
