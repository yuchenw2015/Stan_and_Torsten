...
xdata <- subset(xdata, time > 0 & dose != 0)
...
data <- with(xdata,
             list(
                 ...
                 nObs = nrow(xdata),
                 dose = 1000 * dose[!duplicated(subject)],
                 weight = weight[!duplicated(subject)],
                 subject = subject,
                 time = time,
                 cObs = cobs))
...
init <- function(){
  list(CLHat = exp(rnorm(1, log(10), 0.2)),
       ...
       rho = diag(5),
       sigma = runif(1, 0.5, 2),
       logtheta = matrix(rep(log(c(10, 15, 35, 105, 2)), 
                             ea = data$nSubjects), 
                         nrow = data$nSubjects))
}
...
mcmcDensity(fit, pars = parametersToPlot, nParPerPage = 9, 
            myTheme = theme(text = element_text(size = 12), axis.text = element_text(size = 10)))
...
for(thisDose in doses){
    ...
    p1 <- ggplot(plotdata, aes(x = time, y = cobs))
    p1 <- p1 + geom_point() +
